...
class App extends Component {
  ...
  match = id => {
    const i1 = document.getElementById('i1').src
    console.log(i1)
    const fil = imagesList.filter(each => each.imageUrl === id)
    if (id === fil[0].imageUrl) {
  ...
  reset = () => {
    this.setState({score: 0, activeTabId: tabsList[0].tabId, gameOver: false})
  }

  render() {
    const {score, activeTabId, gameOver} = this.state
    const fil = imagesList.filter(each => each.category === activeTabId)
    return (
      <div className="bg">
        <ul className="nav">
          <img
          ...
          <div className="con1">
            {gameOver ? (
              ...
              <>
                ...
                <ul className="ukl">
                  {fil.map(each => (
                    <AppItem key={each.id} item={each} match={this.match} />
                  ))}
                </ul>
              </>
            )}
